...
// In source build, HOTKEY hooks are no-ops. Just create GuiHotKey without hooks.
// PATCH_HK expands to 0; source-level commas between entries become array separators.
#define HOTKEY_DEFINE(name, txt, keytxt, vkey) ::THPrac::Gui::GuiHotKey name { txt, keytxt, vkey }; static constexpr int _hotkey_dummy_##name[] = {

#define PATCH_HK(addr_, code_) 0
#define EHOOK_HK(addr_, inslen_, ...) 0
