...
// VK_ arrow key constants (Win32 compatible)
// (already defined in thprac_gui_components.h)
	...
	namespace Gui
	{
        ...
        void GameGuiWnd::Update()
            {
                ...
                io.NavInputs[ImGuiNavInput_Activate] = InGameInputGet(VK_RETURN);
                io.NavInputs[ImGuiNavInput_Cancel] = InGameInputGet(VK_ESCAPE);
        ...
        bool GuiCombo::operator()(int skip)
        {
            ...
            if (ImGui::IsItemFocused()) {
                if (KeyboardInputGetSingle(VK_LEFT)) {
                    hasChanged = true;
                    --mCurrent;
                    ...
                    CheckComboItemNew(mSelector, items, -1);
                } else if (KeyboardInputGetSingle(VK_RIGHT)) {
                    hasChanged = true;
                    ++mCurrent;
                    if (skip != -1 && mCurrent == skip)
                        ++mCurrent;
        ...
        bool GuiButton::operator()()
        {
            ...
            if (ImGui::IsItemFocused() && KeyboardInputGetSingle(VK_RETURN))
                res = true;
            return res;
        }
        ...
        bool GuiCheckBox::operator()()
        {
            ...
            if (ImGui::IsItemFocused() && (KeyboardInputGetSingle(VK_LEFT) || KeyboardInputGetSingle(VK_RIGHT))) {
                mToggle = !mToggle;
                pressed = true;
            }
            return pressed;
        ...
        bool GuiHotKey::OnWidgetUpdate()
        {
            ...
            std::string realText;
            if (mStatus) {
                realText = std::string("[") + mKeyText + ": " + text + "]";
        ...
        {
            bool flag = Gui::KeyboardInputGetSingle(mKey);
            if (use_widget)
                flag |= OnWidgetUpdate();
        ...
        void ComboSelect(size_t& out, const char* const* choices, const size_t choices_count, const char* label)
        {
            ...
            if (ImGui::IsItemFocused()) {
                if (Gui::KeyboardInputGetSingle(VK_LEFT) && out > 0) {
                    out--;
                }
                if (Gui::KeyboardInputGetSingle(VK_RIGHT) && out + 1 < choices_count) {
                    out++;
                }
            }
        }
